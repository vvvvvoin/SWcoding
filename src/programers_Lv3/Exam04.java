...
public class Exam04 {
	...
	//static String sentence = "bAaOb";
	static String sentence = "aIaAM";
	//static String sentence = "aHbEbLbLbOacWdOdRdLdDc";
	public static void main(String[] args) {
		...
		for(int i = 0; i < sentenceArray.length; i++) {
			if(Character.isLowerCase(sentence.charAt(i)) && !arrayList.contains(sentence.charAt(i) + "")) {
		...
		ArrayList<Word> hashSet = new ArrayList<Word>();
		...
		for(Word word : hashSet) {
			...
			if(sentenceArray[start] == sentenceArray[end] && word.rule == 1) {
				...
				if(Character.isUpperCase(sentence.charAt(word.start))) word.start++;	
				word.end += 1;
				if(word.end >= sentence.length()) word.end = sentence.length() - 1;
				if(Character.isUpperCase(sentence.charAt(word.end))) word.end--;	
